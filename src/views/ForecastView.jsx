...
export default function ForecastView({ forecastData = [] }) {
  ...
  const filtered = forecastData.filter((item) => {
    const matchesSearch = item.timeLabel.toLowerCase().includes(searchFilter.toLowerCase()) || item.dayLabel.toLowerCase().includes(searchFilter.toLowerCase());
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          ['Scheduled Demand', `${Math.round(totalDemand).toLocaleString()} MWh`, 'CAISO balancing node', 'battery', 0x818cf8],
      ...
      <Glass className="p-5">
        ...
        <div className="relative h-64">
          <svg className="w-full h-full" viewBox={`0 0 ${displayed.length * 36 + 60} 220`} preserveAspectRatio="none">
            ...
            {[40, 90, 140, 190].map((y) => <line key={y} x1="30" y1={y} x2={displayed.length * 36 + 40} y2={y} stroke="#bae6fd" strokeDasharray="3 4" strokeWidth="1" />)}
            <polygon fill="url(#genFill)" points={`40,190 ${displayed.map((d, i) => `${40 + i * 36},${yOf(d.totalGen)}`).join(' ')} ${40 + (displayed.length - 1) * 36},190`} />
          ...
          {hovered != null && displayed[hovered] && (
            <div className="absolute top-2 right-2 glass-chip rounded-xl p-3 text-xs font-mono border border-sky-200 shadow-md">
              ...
              <div className={displayed[hovered].netBalance >= 0 ? 'text-emerald-700 font-bold' : 'text-amber-700 font-bold'}>{displayed[hovered].netBalance} MW</div>
            </div>
          )}
        </div>
      </Glass>
        ...
        <div className="overflow-auto max-h-[380px]">
          <table className="w-full text-left font-mono text-xs">
            <thead className="sticky top-0 bg-sky-100/90 text-slate-700 backdrop-blur-md border-b border-sky-200/80 z-10">
              <tr>{['Time', 'Day', 'Solar', 'Wind', 'Gen', 'Demand', 'Delta', 'm/s', 'Flag'].map((h) => <th key={h} className="py-2 px-2 font-bold text-slate-800">{h}</th>)}</tr>
            </thead>
